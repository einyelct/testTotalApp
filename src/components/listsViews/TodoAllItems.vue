<template>
  <ul class="todo-list">
    <input type="checkbox" class="toggle-all" :checked="getAllNotCompleted" />
    <label @click="checkAll"></label>
    <li
      :class="{
        completed: item.state == 2 && !item.isFocus,
...
export default {
  ...
  computed: {
    ...
    getAllCompleted() {
      let completed = this.task
        .map((item) => {
          if (item.state === 2) return item.id;
        })
  ...
  methods: {
    ...
    changeLabel(item) {
      if (item.label != "") {
        let newItem = this.newItem(item, item.state);
        axios({
          method: "put",
    ...
    checkAll() {
      let listUrl = [];
      if (this.getAllCompleted && !this.getAllNotCompleted) {
        this.task.forEach((element) => {
          if (element.state == 2) {
            element.state = 1;
            listUrl.push({
              url: `http://localhost:3000/todos/${element.id}`,
              data: this.newItem(element, 1),
            });
          }
        });
      } else {
    ...
    // Cambia el estado completado/ viceversa
    checketState(item) {
